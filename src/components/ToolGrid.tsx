...
export default function ToolGrid({ 
  ...
  return (
    <div className="h-full flex flex-col bg-zinc-900/50 backdrop-blur-sm border-r border-zinc-800 transition-all duration-300">
        ...
        <div className={cn("space-y-6", isCollapsed ? "px-2" : "px-3")}>
          {tools.map((category, idx) => {
            ...
            if (isCollapsed) {
              ...
              return (
                <div key={idx} className="space-y-2 border-b border-zinc-800 pb-2 last:border-0">
                  {category.map((tool) => (
                    <button
                      key={tool.id}
            ...
            return (
              <div key={idx}>
                ...
                >
                  <h3 className="text-xs font-semibold text-zinc-500 uppercase tracking-wider group-hover:text-zinc-300 transition-colors">
