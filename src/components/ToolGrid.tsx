...
export default function ToolGrid({ 
  ...
  return (
    <div className="h-full flex flex-col bg-zinc-50 dark:bg-zinc-950 backdrop-blur-sm border-r border-zinc-200 dark:border-white/10 transition-all duration-300">
        ...
        <div className={cn("space-y-6", isCollapsed ? "px-2" : "px-3")}>
          {tools.map((category, idx) => {
            ...
            if (isCollapsed) {
              ...
              return (
                <div key={idx} className="space-y-2 border-b border-zinc-200 dark:border-white/10 pb-2 last:border-0">
                  {category.map((tool) => (
                    <button
                      key={tool.id}
                      ...
                      className={cn(
                        ...
                        selectedToolId === tool.id
                          ...
                          : "text-zinc-500 dark:text-zinc-400 hover:bg-zinc-200 dark:hover:bg-zinc-800 hover:text-zinc-900 dark:hover:text-zinc-200"
                      )}
                    >
            ...
            return (
              <div key={idx}>
                ...
                >
                  <h3 className="text-xs font-semibold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider group-hover:text-zinc-700 dark:group-hover:text-zinc-300 transition-colors">
                    {t(categoryKey as any)}
                  </h3>
                  {isExpanded ? (
                    <ChevronDown size={14} className="text-zinc-500 dark:text-zinc-600 group-hover:text-zinc-700 dark:group-hover:text-zinc-400" />
                  ) : (
                    <ChevronRight size={14} className="text-zinc-500 dark:text-zinc-600 group-hover:text-zinc-700 dark:group-hover:text-zinc-400" />
                  )}
                </button>
